...
_start_time_ctx_var: ContextVar[Optional[datetime]] = ContextVar(
    START_TIME_CTX_KEY, default=None
)
_end_time_ctx_var: ContextVar[Optional[datetime]] = ContextVar(
    END_TIME_CTX_KEY, default=None
)
...
class RequestContextLogMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint):
        _correlation_id_ctx_var.set(
            request.headers.get(CORRELATION_ID_HEADER, str(uuid4()))
        )
