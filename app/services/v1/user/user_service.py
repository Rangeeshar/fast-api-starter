...
from app.services.v1.user.user_schema import UserRequest, UserResponse
...
def get_users(db: Session) -> List[UserResponse]:
    users: Optional[List[User]] = user_repository.get(db=db)
...
def validate_user_id(id: UUID, db: Session):
    ...
    if not user:
        raise RequestError(
            status_code=HTTPStatus.NOT_FOUND, error_code=ErrorCode.INCORRECT_USER_ID
        )
