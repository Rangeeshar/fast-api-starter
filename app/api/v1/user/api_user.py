...
from app.services.v1.user.user_schema import UserResponse, UserRequest
from app.services.v1.user import user_service
...
async def create_user(
    user_request: UserRequest, request: Request, db: Session = Depends(get_db)
) -> UserResponse:
    user: Optional[User] = user_service.create_user(
        user_request=user_request, request=request, db=db
    )
    return user
...
async def update_user(
    id: UUID, user_request: UserRequest, request: Request, db: Session = Depends(get_db)
) -> UserResponse:
    user: Optional[User] = user_service.update_user(
        id=id, user_request=user_request, request=request, db=db
    )
    return user


@api_router.delete("/{id}", status_code=HTTPStatus.OK)
async def delete_user_by_id(id: UUID, db: Session = Depends(get_db)):
